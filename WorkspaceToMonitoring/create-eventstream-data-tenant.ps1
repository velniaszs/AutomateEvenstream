...
$sourceTenantName = "TenantWorkspaceEvents"
...
if ($CheckWorkspace -eq "yes") {
    ...
    $existingName = $jsonContent.sources | Where-Object { $_.name -eq $sourceTenantName }
    if ($existingName) {
        Write-Warning "Source with name '$sourceTenantName' already exists."
    }
...
if ($stream) {
    ...
    $stream.inputNodes += [PSCustomObject]@{ name = $sourceTenantName }
    #Write-Host "Added '$sourceTenantName' to stream '$capacityName-stream'."
} else {
