...
param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$rmWorkspaceName = "ab-test5", #Name of the workspace to be added

    [Parameter(Mandatory = $false)]
    [string]$rmWorkspaceId = "f085bd5f-211d-4e78-8795-0db1a1e464f1", #WorkspaceId of the workspace to be removed
...
Write-Host "--- Checking if workspace to be deleted exists in local evenstream definition ---"
...
if ($wsExists) {
    Write-Host "--- Step 4: Removing workspace info from evenstream json file definition ---"
    $createEventstreamDataScript = Join-Path $PSScriptRoot "\remove-eventstream-data.ps1"
    $evenstreamDeleted = & $createEventstreamDataScript -sourceWorkspaceName $rmWorkspaceName -sourceWorkspaceId $rmWorkspaceId -jsonPath "$PSScriptRoot\input\eventstream.json" -capacityName $capacityName -destinationWorkspaceId $WorkspaceId -AuthToken $fabricToken

    IF (-not $evenstreamDeleted) {
    Write-Host "--- Step 5: Updating Eventstream Definition ---"
    ...
    Write-Host "Workspace does not exist in definition. Skipping Step 4 and 5."
}
